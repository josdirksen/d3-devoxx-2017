...
function process(canvasId, srcId, blurFactor, blockSize, ratio, threshold) {
    ...
    return getPoints(canvasId, blockSize, 0.2, threshold, ratio);
}
...
var drawTriangle = function(canvas, voronoi, sites, original) {
    ...
    var sitesSanatized = _.uniqWith(sites, function(a, b) {
        return ((Math.abs(a[0]-b[0])) < 3) &&
            ((Math.abs(a[1]-b[1])) < 3)
    });

    console.log(sitesSanatized)
    var polygons = voronoi(sitesSanatized).triangles();
